...
(function(e) {
	e.fn.seatCharts = function(t) {
			...
			u = function(t, n) {
				return function(i) {
					...
					s.settings.$node.attr({
						...
						"available",
					// }).text(s.settings.label).addClass(["seatCharts-seat", "seatCharts-cell",
					// 	"available"
					].concat(s.settings.classes, typeof n.seats[s.settings.character] ==
						"undefined" ? [] : n.seats[s.settings.character].classes).join(" "));
					s.data = function() {
					...
					s.style = function() {
						return arguments.length == 1 ? function(e) {
							...
							n.animate ? s.settings.$node.switchClass(t, e, 200) : s.settings.$node
								.removeClass(t).addClass(e);
							return s.settings.style = e
						}(arguments[0]) : s.settings.style
					};
					s.status = function() {
						return s.settings.status = arguments.length == 1 ? s.style(arguments[0]) : s
							.settings.status
					};
					(function(i, o, u) {
						e.each(["click", "focus", "blur"], function(e, a) {
							s[a] = function() {
								...
								return s.style(typeof i[o][a] === "function" ? i[o][a]
									.apply(u) : n[a].apply(u))
							}
						})
					})(n.seats, s.settings.character, s);
					s.node().on("click", s.click).on("mouseenter", s.focus).on("mouseleave", s.blur).on(
						"keydown",
						function(e, n) {
							return function(i) {
								var s;
									...
									case 38:
										...
										s = function o(e, t, r) {
											...
											if (!e.index(r) && i.which == 38) {
												u = e.last()
											} else if (e.index(r) == e.length - 1 && i.which ==
												40) {
												u = e.first()
											} else {
												u = e.eq(e.index(r) + (i.which == 38 ? -1 : +1))
											}
											s = u.find(".seatCharts-seat,.seatCharts-space").eq(
												t.index(n));
											return s.hasClass("seatCharts-space") ? o(e, t, u) :
												s
										}(n.parents(".seatCharts-container").find(
												".seatCharts-row:not(.seatCharts-header)"), n
											.parents(".seatCharts-row:first").find(
												".seatCharts-seat,.seatCharts-space"), n
									...
									case 39:
										...
										s = function(e) {
											if (!e.index(n) && i.which == 37) {
												return e.last()
											} else if (e.index(n) == e.length - 1 && i.which ==
												39) {
												return e.first()
											} else {
												return e.eq(e.index(n) + (i.which == 37 ? -1 : +
													1))
											}
										}(n.parents(".seatCharts-container:first").find(
		...
		e.each(o.map, function(t, s) {
			...
			if (o.naming.left) {
				a.append(e("<div></div>").addClass("seatCharts-cell seatCharts-space").text(o.naming
					.rows[t]))
			}
		...
		o.legend.items.length ? function(t) {
			...
			e.each(t.items, function(t, n) {
				i.append(e("<li></li>").addClass("seatCharts-legendItem").append(e("<div></div>")
					.addClass(["seatCharts-seat", "seatCharts-cell", n[1]].concat(o.classes,
						typeof o.seats[n[0]] == "undefined" ? [] : o.seats[n[0]].classes
						).join(" "))).append(e("<span></span>").addClass(
					"seatCharts-legendDescription").text(n[2])))
			});
			return r
		}(o.legend) : null;
		n.attr({
			...
			set: function f() {
				...
				return {
					...
					status: function() {
						var t = arguments,
							n = this;
						return this.length == 1 && t.length == 0 ? this.seats[0].status() :
							function() {
